...
def detect_anomaly(df,date,rf):
	...
	final_price_array,final_date_array=[],[]
	for i in range(0,len(price_array)-43,14):
		#print(i)
		l1=price_array[i:i+43]
		l2=date_array[i:i+43]
		final_price_array.append(l1)
		final_date_array.append(l2)
	final_price_array=np.array(final_price_array)
	final_date_array=np.array(final_date_array)
	#print(final_date_array.shape)
	pred=rf.predict(final_price_array)
	for i in range(len(pred)):
		print(pred[i])
		print(final_date_array[i][21])

	#print(pred)

df=pd.read_csv('/home/ictd/nishant/research/Low_Price/meeting/azadpur_price_arimax.csv')
